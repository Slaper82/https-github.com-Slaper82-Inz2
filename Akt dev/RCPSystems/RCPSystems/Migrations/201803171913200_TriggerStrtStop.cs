namespace RCPSystems.Migrations
{
    ...
    public partial class TriggerStrtStop : DbMigration
    {
            ...
            Sql(@"IF NOT EXISTS (select * from sys.objects where type = 'TR' and name = 'PARY_TRANS')  EXEC dbo.sp_executesql @statement = N'
                ...
                AS       
                    BEGIN
                        ...
                        IF(Select IdTransType from inserted) = 1 and(select count(*) from rcpPairsIO where CONVERT(Date, Start) = CONVERT(Date, (Select DateAndTime from inserted)) and IdUser=(select IdUser from inserted) and Stop is null ) = 0
                ...
                AS
                    BEGIN
		                 if(Select IdTransType from inserted)=2 and(select count(*) from rcpPairsIO where CONVERT(Date, Start)= CONVERT(Date, (Select DateAndTime from inserted)) and IdUser=(select IdUser from inserted) and[Stop] is null)=1
			                begin
				                ...
				                declare @dataWej as Datetime set @dataWej=(Select top 1 Start from rcpPairsIO where CONVERT(Date, Start)=CONVERT(Date, @dataWsad) and IdUser=@id order by[Stop] desc)
				                if(@dataWsad>@dataWej)
					                Begin
                                        update rcpPairsIO
                                        set[Stop] = @dataWsad
